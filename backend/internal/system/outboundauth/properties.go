...
// ToProperties renders cfg for a consumer's property bag. The descriptor decides which values
// are secret, so cmodels.NewProperty encrypts them on construction. A field with a blank value
// is skipped, matching how the sender services treat a missing property.
//
// A field the method does not declare is dropped rather than stored, which guarantees nothing
// can reach storage unencrypted by being named something the descriptors do not know. That
// makes an undeclared property silently ignored rather than rejected, consistent with how the
// rest of the API treats unknown request fields.
func ToProperties(cfg Config) ([]cmodels.Property, error) {
	...
	props := make([]cmodels.Property, 0, len(descriptor.Fields)+1)
	...
	for _, fieldDescriptor := range descriptor.Fields {
		...
		value := cfg.Get(fieldDescriptor.Name)
		if !fieldDescriptor.Credential {
			value = strings.TrimSpace(value)
		}
		if value == "" {
			continue
		}
		prop, err := cmodels.NewProperty(PropertyKey(fieldDescriptor.Name), value, fieldDescriptor.Credential)
		if err != nil {
			return nil, fmt.Errorf("failed to build property %s: %w",
				PropertyKey(fieldDescriptor.Name), err)
		}
...
func FromValues(values map[string]string) Config {
	...
	for _, fieldDescriptor := range descriptor.Fields {
		if value, exists := values[PropertyKey(fieldDescriptor.Name)]; exists {
			cfg.Properties[fieldDescriptor.Name] = value
		}
	}
