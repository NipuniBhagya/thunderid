...
func (s *OutboundAuthTestSuite) TestDescriptorLookup() {
	descriptor, ok := Descriptor(TypeBasic)
	s.True(ok)
	s.Equal(TypeBasic, descriptor.Type)
	s.Require().Len(descriptor.Fields, 2)

	// Render order is part of the contract: a credential form shows the username first.
	s.Equal(FieldBasicUsername, descriptor.Fields[0].Name)
	s.Equal(FieldBasicPassword, descriptor.Fields[1].Name)
	s.False(descriptor.Fields[0].Credential)
	s.True(descriptor.Fields[1].Credential)

	_, ok = Descriptor(Type("bearer"))
	s.False(ok)
}

func (s *OutboundAuthTestSuite) TestDescriptorsPreservesCallerOrderAndSkipsUnknown() {
	result := Descriptors([]Type{TypeBasic, Type("bearer"), TypeNone})
...
func (s *OutboundAuthTestSuite) TestDescriptorsServesFieldlessMethodAsEmptyArray() {
	result := Descriptors([]Type{TypeNone})
...
// The remaining descriptor features have no built-in method using them yet, so they are
// exercised against a temporary descriptor. This is what a future method relies on.
func (s *OutboundAuthTestSuite) withTemporaryDescriptor(descriptor MethodDescriptor, body func()) {
	original := descriptors
	descriptors = append(append([]MethodDescriptor{}, descriptors...), descriptor)
...
func (s *OutboundAuthTestSuite) TestValidateEnforcesEnum() {
	...
	s.withTemporaryDescriptor(MethodDescriptor{
		Type:        apiKey,
		DisplayName: "API key",
