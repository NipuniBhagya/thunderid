...
//
// Method descriptors are the single source of truth. A descriptor decides the property key a
// value is stored under, whether it is required, whether it is a secret, and how a console
// renders it, so adding a method means adding a descriptor rather than touching every caller.
//
...
// FieldDescriptor describes one credential field of an authentication method. It is the single
// source of truth for that field: the property key it is stored under, whether a value is
// required, whether the value is a secret, and how a console renders it. The JSON tags speak
// the console's property definition vocabulary so a metadata endpoint can serve descriptors
// verbatim.
...
// MethodDescriptor describes one authentication method end to end: its discriminator and its
// credential fields in the order a console should render them.
type MethodDescriptor struct {
	...
	Fields []FieldDescriptor `json:"properties"`
	// Validate, when set, checks rules internal to this method that the field descriptors
	// cannot express, such as requiring exactly one of two alternative credentials. Rules that
	// depend on the transport belong to the caller instead.
...
// keyed by FieldDescriptor.Name. A zero Config authenticates nothing.
type Config struct {
	Type       Type
...
// empty value yields TypeNone. The second return reports whether value named a known type at
// all; whether that type is permitted in a given context is Validate's business.
