...
// come from the method's descriptor, so the paths are derived rather than hardcoded. That is
// what keeps a new authentication method from needing a change here.
func authenticationResourceRules(
	...
	descriptor, ok := outboundauth.Descriptor(authType)
	if !ok {
		return &declarativeresource.ResourceRules{}
	}
