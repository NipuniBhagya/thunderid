...
// order a console should offer them. The descriptors are served verbatim from the outboundauth
...
type connectionAuthMetaSection struct {
	Methods []outboundauth.MethodDescriptor `json:"methods"`
}
...
func connectionMeta(vendor string) (*connectionMetaResponse, *tidcommon.ServiceError) {
	...
	return &connectionMetaResponse{
		...
		Authentication: connectionAuthMetaSection{
			Methods: outboundauth.Descriptors(authTypes),
		},
	}, nil
}
