...
// discriminator plus a property bag whose keys come from the method's descriptor. Keeping the
// bag generic is what lets a new authentication method reach the console without a change here
// or a new schema. On a response the secret values are already masked.
...
// properties. The descriptors decide which values are secret, so the password is encrypted on
// construction and masked on every later read.
