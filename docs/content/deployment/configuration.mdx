...
:::warning[Removed]
The `email.smtp` block is no longer read. <ProductName /> sends email through an **email provider**, a connection you create from the Console or the connections API, and each email step in a flow names the provider it sends through. See [Configure an Email Provider](../../guides/notifications/email-providers) and [Migrate from deployment.yaml SMTP](../../guides/smtp-server/smtp-server-configuration).
:::
