...
- An email provider must be configured in <ProductName />, and selected on the executor through its `senderId` property. See [Configure an Email Provider](../../notifications/email-providers). There is no deployment-wide default: the executor returns failure status when `senderId` is absent or names a provider that does not exist.
...
| `senderId` | Email Provider | Yes | | The ID of the email provider that sends the message. Enforced when the step runs, not when the flow is saved, so a flow authored before any provider exists still saves. |
...
**Failure conditions:**
- `senderId` not set in node properties, or no email provider with that ID
