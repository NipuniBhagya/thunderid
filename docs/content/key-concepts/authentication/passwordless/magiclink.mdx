...
- **Email Provider**: An email provider must be configured to send the emails. See [Configure an Email Provider](../../../../guides/notifications/email-providers).
...
1) **Extract the token.** Read the `token` from the URL query parameters.

2) **Continue the flow.** Submit the token back to the flow engine for verification.
